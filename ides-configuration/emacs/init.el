...
;;; ---------------------------------------------------------------
;; 0. Package bootstrap (straight.el + use-package)
...
(use-package auctex
  ...
  (TeX-engine 'pdflatex)
  ;; Route chktex (flymake backend) through the Docker wrapper
  (TeX-chktex-program "docker-chktex")
  :config
  ;; Route compilation through the Docker wrapper (mirrors the Makefile)
...
(use-package flyspell
  ...
  (ispell-dictionary "pt_BR"))   ; switch with M-x ispell-change-dictionary
...
(use-package project
  :straight nil
  :config
  ;; Register the repo root so C-x p commands work immediately
